...
ArrayList<Departments> dep_list = (ArrayList<Departments>) session.getAttribute("dep_list");
HashMap<Integer,String> sch_map = (HashMap<Integer,String>) session.getAttribute("sch_map");
...
<%
	...
	<div class="w3-main" style="margin-left: 300px; margin-top: 43px;">
		...
		<header class="w3-container" style="padding-top: 22px">
			<h2>
				<b><i class="fas fa-laptop-code"></i> Department List</b>
			</h2>
		</header>
		<div class="panel-body">
			<div class="input-group">
				<input class="form-control" type="text"
					placeholder="Search for department names..." id="myInput"
			...
			<br>
			<form action="AdminDepartment" method="post">
				<table id="table" class="w3-table-all w3-hoverable" cellpadding="0"
					...
					<thead>
						...
						<tr>
							<td class="w3-center"><input size="1" class="form-control w3-center" type="number" name="id"/></td>
							<td class="w3-center"><input size="10" class="form-control w3-center" type="text" name="name" required/></td>
							<td><select class="form-control" name="sch_id" required>
									<option class="hidden" selected disabled>Please select school *</option>
									<%
									for (Integer key : sch_map.keySet()) {
									%>
									<option value="<%=key%>"><%=sch_map.get(key)%></option>
									<%
									}
									%>
								</select></td>
						...
						%>
						<tr>
							<td class="w3-center"><%=dep_list.get(i).getId()%></td>
							<td class="w3-center"><%=dep_list.get(i).getName()%></td>
							<td><%=sch_map.get(dep_list.get(i).getSch_id())%></td>
							<td class="w3-center"><a href="AdminDepartment?action=delete&dep_id=<%=dep_list.get(i).getId()%>"><i
