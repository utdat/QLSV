...
ArrayList<StuSubject> stu_point = (ArrayList<StuSubject>) session.getAttribute("stu_point");
ArrayList<Subjects> stu_sub = (ArrayList<Subjects>) session.getAttribute("stu_sub");
...
<%
	...
	<nav class="w3-sidebar w3-collapse w3-white "
		...
		<div class="w3-bar-block">
			<a href="<c:url value="StudentScore"/>"
			...
			<table id="table" class="w3-table-all w3-hoverable" cellpadding="0"
				...
				<thead>
					<tr class="w3-green">
						...
						<th class="w3-center">Endterm</th>
						<th class="w3-center">Delete</th>
					</tr>

					<%
					for (int i = 0; i < stu_sub.size(); i++) {
					%>
					<tr>
						<td class="w3-center"><%=stu_sub.get(i).getId()%></td>
						<td><%=stu_sub.get(i).getName()%></td>
						<td><%=stu_sub.get(i).getSchedule()%></td>
						<td class="w3-center"><%=stu_sub.get(i).getRoom()%></td>
						<td class="w3-center"><%=stu_sub.get(i).getStartdate()%></td>
						<td class="w3-center">
							<%
							if (stu_point.get(i).getMidterm() >= 0) {
							%> <%=stu_point.get(i).getMidterm()%>
							<%
							}
							%>
						</td>
						<td class="w3-center">
							<%
							if (stu_point.get(i).getEndterm() >= 0) {
							%> <%=stu_point.get(i).getEndterm()%>
							<%
							}
							%>
						</td>
						<td class="w3-center">
							<%
							if (stu_sub.get(i).getStartdate().after(new Date())) {
							%> <a
							href="StudentScore?action=delete&sub_id=<%=stu_sub.get(i).getId()%>">
