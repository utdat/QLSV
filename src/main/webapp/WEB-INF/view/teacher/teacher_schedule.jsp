...
HashMap<Subjects, String> sche_map = (HashMap<Subjects, String>) session.getAttribute("sche_map");
		...
		<div class="panel-body">
			...
			<table id="table" class="w3-table-all w3-hoverable" cellpadding="0"
				...
				<thead>
					...
					<%
					for (Subjects key : sche_map.keySet()) {
					%>
					<tr>
						<td class="w3-center"><%=key.getId()%></td>
						<td><%=key.getName()%></td>
						<td><%=key.getSchedule()%></td>
						<td><%=sche_map.get(key)%></td>
						<td class="w3-center"><%=key.getRoom()%></td>
						<td class="w3-center"><%=key.getStartdate()%></td>
						<td class="w3-center"><%=key.getEnddate()%></td>
						<td class="w3-center">
							<%
							if (key.getStartdate().before(new Date()) && key.getEnddate().after(new Date())) {
							%> <a href="TeacherMark?sub_id=<%=key.getId()%>"><i
